...
from lab6Scripts.polygonsFilling import PolygonsFilling
from lab7Scripts.triangulation import triangulate, generate_points
from lab7Scripts.diagram import diagram
import time
import random
...
def draw_polygon(algorithm, x_coords, y_coords, canvas, info_label):
    ...
    if hull and len(hull) >= 2:
        ...
        ax.plot(xs, ys, marker='o', linestyle='-', color='k', zorder=3)
...
def draw_intervals_on_axis(ax, intervals: list):
    x_all = []
    y_all = []
    for interval in intervals:
        if len(interval) == 1:
            x, y = interval[0]
            rect = plt.Rectangle((x, y), 1, 1, color='blue', ec='black', zorder=1)
            ax.add_patch(rect)
            x_all.append(x)
            y_all.append(y)
        else:
...
def draw_intervals(ax, intervals: list):
    for idx, interval in enumerate(intervals):
        try:
            if isinstance(interval, list) and len(interval) == 2:
                pt1, pt2 = interval[0], interval[1]
                x_start, y = pt1
                x_end, _ = pt2
                for x in range(int(x_start), int(x_end) + 1):
            ...
            print(f"[draw_pixels_on_axis] Ошибка при обработке интервала {idx}: {interval}. Ошибка: {e}")
    ax.figure.canvas.draw_idle()


def draw_pixels_on_axis(ax, pixels: list):
    x_all = []
    y_all = []
    for (x, y) in pixels:
        x_all.append(x)
...
def open_new_polygon_window(default_algorithm=None, fill_algorithm=None):
    poly_window = Toplevel()
    poly_window.title("Отрисовка выпуклой оболочки")
    poly_window.geometry("900x700")
    input_frame = tk.Frame(poly_window)
    ...
    btn_fill = Button(button_frame, text="Заполнить полигон", command=lambda: on_fill())
    ...
    def on_graham():
        ...
        try:
            ...
            for widget in canvas_frame.winfo_children():
                widget.destroy()
            hull_poly, pc_new, poly_fig, poly_ax = draw_polygon('graham', x_coords, y_coords, canvas_frame, info_label)
            btn_fill.pack(side=tk.LEFT, padx=5)
        except Exception as e:
            info_label.config(text="Ошибка ввода координат: " + str(e))
    ...
    def on_fill():
        nonlocal hull_poly, pc_new, poly_fig, poly_ax
        if hull_poly is None:
            info_label.config(text="Сначала постройте полигон!")
            return
        pf = PolygonsFilling(hull_poly.copy())
        if fill_algorithm in ['simple', 'line_by_line']:
            seed_x = int(sum([p[0] for p in hull_poly]) / len(hull_poly))
            seed_y = int(sum([p[1] for p in hull_poly]) / len(hull_poly))
            seed = [seed_x, seed_y]
        try:
            if fill_algorithm in ['ordered_list_of_edges', 'ordered_list_with_active_list_of_edges']:
                if fill_algorithm == 'ordered_list_of_edges':
                    intervals = pf.with_ordered_list_of_edges()
                else:
                    intervals = pf.ordered_list_using_active_edges()
                draw_intervals(poly_ax, intervals)
            elif fill_algorithm in ['simple', 'line_by_line']:
                if fill_algorithm == 'simple':
                    pixels = pf.simple_seeded_filling(seed)
                else:
                    pixels = pf.seeded_filling_line_by_line(seed)
                draw_pixels_on_axis(poly_ax, pixels)
            poly_fig.canvas.draw_idle()
        except Exception as e:
            info_label.config(text="Ошибка заполнения полигона: " + str(e))
...
def open_new_polygon_debug_window(default_algorithm=None, fill_algorithm=None):
    poly_window = Toplevel()
    poly_window.title("Отладка заполнения полигона")
    poly_window.geometry("900x700")

    style = ttk.Style(poly_window)
    style.theme_use('clam')
    style.configure("TCombobox", foreground="black", background="white", padding=5, font=("Arial", 10))
    fill_mode_var = tk.StringVar(value="ручной")
    fill_mode_frame = tk.Frame(poly_window)
    fill_mode_frame.pack(pady=5)
    lbl_mode = Label(fill_mode_frame, text="Режим заполнения:", font=("Arial", 10))
    lbl_mode.pack(side=tk.LEFT, padx=5)
    fill_mode_combo = ttk.Combobox(fill_mode_frame, textvariable=fill_mode_var,
                                   state="readonly", values=["ручной", "автоматический"],
                                   font=("Arial", 9), width=15)
    fill_mode_combo.pack(side=tk.LEFT, padx=5)
    ...
    poly_ax = None
    debug_gen = None
    btn_build = Button(button_frame, text="Построить", command=lambda: on_build())
    btn_build.pack(side=tk.LEFT, padx=5)
    btn_step = Button(button_frame, text="Следующий шаг", state=tk.DISABLED, command=lambda: on_step())
    btn_step.pack(side=tk.LEFT, padx=5)
    btn_auto = Button(button_frame, text="Автоматическое заполнение", state=tk.DISABLED, command=lambda: auto_fill())
    btn_auto.pack(side=tk.LEFT, padx=5)

    def on_build():
        nonlocal hull_poly, pc_new, poly_fig, poly_ax, debug_gen, fill_algorithm
        try:
            x_coords = [int(x.strip()) for x in entry_x.get().split(",") if x.strip() != ""]
            y_coords = [int(y.strip()) for y in entry_y.get().split(",") if y.strip() != ""]
            if len(x_coords) != len(y_coords):
            ...
            for widget in canvas_frame.winfo_children():
                widget.destroy()
            if default_algorithm == 'graham':
                hull_poly, pc_new, poly_fig, poly_ax = draw_polygon('graham', x_coords, y_coords, canvas_frame,
                                                                    info_label)
            elif default_algorithm == 'jarvis':
                hull_poly, pc_new, poly_fig, poly_ax = draw_polygon('jarvis', x_coords, y_coords, canvas_frame,
                                                                    info_label)
            else:
                info_label.config(text="Не выбран алгоритм построения!")
                return
            pf = PolygonsFilling(hull_poly.copy())
            seed_x = int(sum([p[0] for p in hull_poly]) / len(hull_poly))
            seed_y = int(sum([p[1] for p in hull_poly]) / len(hull_poly))
            seed = [seed_x, seed_y]
            if fill_algorithm == 'simple':
                debug_gen = pf.debug_simple_seeded_filling(seed)
            elif fill_algorithm == 'line_by_line':
                debug_gen = pf.debug_seeded_filling_line_by_line(seed)
            elif fill_algorithm == 'ordered_list_of_edges':
                debug_gen = pf.debug_ordered_list_of_edges()
            elif fill_algorithm == 'ordered_list_with_active_list_of_edges':
                debug_gen = pf.debug_ordered_list_using_active_edges()
            else:
                info_label.config(text="Не выбран алгоритм заполнения!")
                return
            btn_step.config(state=tk.NORMAL)
            btn_auto.config(state=tk.NORMAL)
            info_label.config(text="Построен полигон. Нажмите 'Следующий шаг' для пошагового заполнения.")
        except Exception as e:
            info_label.config(text="Ошибка ввода координат: " + str(e))

    def on_step():
        nonlocal debug_gen, poly_fig, poly_ax, fill_algorithm
        try:
            debug_step = next(debug_gen)
            if fill_algorithm in ['ordered_list_of_edges', 'ordered_list_with_active_list_of_edges']:
                draw_intervals(poly_ax, debug_step)
            elif fill_algorithm in ['simple', 'line_by_line']:
                draw_pixels_on_axis(poly_ax, debug_step)
            else:
                print("[on_step] Неизвестный метод заполнения:", fill_algorithm)
            poly_fig.canvas.draw_idle()
        except StopIteration:
            btn_step.config(state=tk.DISABLED)
            btn_auto.config(state=tk.DISABLED)
            info_label.config(text="Заполнение завершено")
        except Exception as e:
            print(f"[on_step] Ошибка: {e}")
            info_label.config(text="Ошибка в процессе заполнения: " + str(e))

    def auto_fill():
        nonlocal debug_gen, poly_fig, poly_ax, fill_mode_var, fill_algorithm
        try:
            if fill_mode_var.get() == "автоматический":
                debug_step = next(debug_gen)
                if fill_algorithm in ['ordered_list_of_edges', 'ordered_list_with_active_list_of_edges']:
                    draw_intervals(poly_ax, debug_step)
                elif fill_algorithm in ['simple', 'line_by_line']:
                    ...
                    print("[auto_fill] Неизвестный метод заполнения:", fill_algorithm)
                poly_fig.canvas.draw_idle()
                poly_window.after(1000, auto_fill)
        except StopIteration:
            btn_step.config(state=tk.DISABLED)
            btn_auto.config(state=tk.DISABLED)
            info_label.config(text="Заполнение завершено")
        except Exception as e:
            print(f"[auto_fill] Ошибка: {e}")
            info_label.config(text="Ошибка в процессе заполнения: " + str(e))

    poly_window.mainloop()


def draw_triangulation(points, triangles, canvas):
    fig, ax = plt.subplots()
    ax.set_aspect('equal')
    ax.scatter(points[:, 0], points[:, 1], color='blue')
    for i in range(0, len(triangles), 3):
        x = [points[triangles[i], 0], points[triangles[i + 1], 0], points[triangles[i + 2], 0], points[triangles[i], 0]]
        y = [points[triangles[i], 1], points[triangles[i + 1], 1], points[triangles[i + 2], 1], points[triangles[i], 1]]
...
def open_triangulation_window():
    triangulation_window = Toplevel()
    triangulation_window.title("Триангуляция Делоне")
    triangulation_window.geometry("630x600")

    frame_point2 = tk.Frame(triangulation_window)
    frame_point2.pack(pady=5)

    # Поле ввода для количества точек
    lbl_x2 = Label(frame_point2, text="Количество точек:")
    lbl_x2.pack(side=tk.LEFT, padx=5)
    entry_x2 = Entry(frame_point2, width=10)
    entry_x2.pack(side=tk.LEFT, padx=5)

    canvas_frame = tk.Frame(triangulation_window, bg="white", width=300, height=200)
    canvas_frame.pack(pady=10, fill=tk.BOTH, expand=True)

    info_label = tk.Label(triangulation_window, text="", font=("Arial", 12), justify="left")
    info_label.pack()
    ...
    def on_ok():
        try:
            ...
            for widget in canvas_frame.winfo_children():
                widget.destroy()
            draw_triangulation(points, triangles, canvas_frame)
        except ValueError:
            print("Введите корректные числовые значения!")

    btn_ok = Button(triangulation_window, text="Ок", command=on_ok)
    btn_ok.pack(pady=10)


def plot_voronoi(points, container):
    voronoi_cells, box = diagram(points)
    ...
    for cell in voronoi_cells:
        ...
        ax.plot(xs, ys, color='black', marker='o')
    box_polygon = box + [box[0]]
    xs_box = [p[0] for p in box_polygon]
    ys_box = [p[1] for p in box_polygon]
    ax.plot(xs_box, ys_box, marker='o', color='black', linestyle='--')
...
def open_input_window():
    win = Toplevel()
    win.title("Ввод данных для диаграммы Вороного")
    win.geometry("300x150")
    lbl = Label(win, text="Введите количество точек:")
    lbl.pack(pady=10)
    entry_num = Entry(win, width=10)
    ...
    def proceed():
        try:
            ...
            if num < 1:
                raise ValueError
            open_voronoi_input_window(num)
            win.destroy()
        except ValueError:
            error_label.config(text="Введите целое положительное число!")
    btn = Button(win, text="Далее", command=proceed)
    btn.pack(pady=5)


def open_voronoi_input_window(num_points: int):
    input_win = Toplevel()
    input_win.title("Ввод координат точек")
    input_win.geometry("400x250")
    lbl = Label(input_win, text="Введите координаты для каждой точки (X, Y):")
    lbl.pack(pady=5)
    entries = []
    for i in range(num_points):
        frm = Frame(input_win)
        frm.pack(pady=3)
        lbl_pt = Label(frm, text=f"Точка {i + 1}")
        lbl_pt.pack(side=tk.LEFT, padx=5)
        lbl_x = Label(frm, text="X:")
        lbl_x.pack(side=tk.LEFT)
        entry_x = Entry(frm, width=5)
        entry_x.pack(side=tk.LEFT, padx=5)
        lbl_y = Label(frm, text="Y:")
        lbl_y.pack(side=tk.LEFT)
        entry_y = Entry(frm, width=5)
        entry_y.pack(side=tk.LEFT, padx=5)
        entries.append((entry_x, entry_y))
    error_lbl = Label(input_win, text="", fg="red")
    error_lbl.pack()
    def on_build():
        try:
            pts = []
            for ex, ey in entries:
                ...
                pts.append([x, y])
            input_win.destroy()
            open_voronoi_diagram_window(pts)
        except ValueError:
            error_lbl.config(text="Введите корректные числовые значения!")
    btn_build = Button(input_win, text="Построить диаграмму", command=on_build)
    btn_build.pack(pady=10)


def open_voronoi_diagram_window(points):
    diagram_win = Toplevel()
    diagram_win.title("Диаграмма Вороного")
    diagram_win.geometry("600x600")
    cells, box = diagram(points)
    fig = Figure(figsize=(5, 5), dpi=100)
    ax = fig.add_subplot(111)
    for cell in cells:
        poly = cell + [cell[0]]
        ...
        ax.fill(xs, ys, facecolor=color, alpha=0.4, edgecolor="black")
        ax.plot(xs, ys, color='black', marker='o')
    box_poly = box + [box[0]]
    xs_box = [p[0] for p in box_poly]
    ys_box = [p[1] for p in box_poly]
    ax.plot(xs_box, ys_box, marker='o', color='black', linestyle='--')
    ax.scatter(*zip(*points), color='black')
    ax.set_aspect('equal')
    ax.grid(True)
    canvas = FigureCanvasTkAgg(fig, master=diagram_win)
    canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
    canvas.draw()
...
def create_window():
    ...
    btn_segmentss.menu.add_command(label="Алгоритм растровой развёртки с упорядоченным списком рёбер", command=lambda:open_new_polygon_debug_window(default_algorithm='graham', fill_algorithm='ordered_list_of_edges'))
    btn_segmentss.menu.add_command(label="Алгоритм растровой развёртки с упорядоченным списком рёбер, использующий САР", command=lambda:open_new_polygon_debug_window(default_algorithm='graham', fill_algorithm='ordered_list_with_active_list_of_edges'))
    btn_segmentss.menu.add_command(label="Простой алгоритм заполнения с затравкой", command=lambda:open_new_polygon_debug_window(default_algorithm='graham', fill_algorithm='simple'))
    btn_segmentss.menu.add_command(label="Построчный алгоритм заполнения с затравкой", command=lambda:open_new_polygon_debug_window(default_algorithm='graham', fill_algorithm='line_by_line'))
    ...
    # Кнопка "Алгоритмы заполнения полигонов"
    btn_segments5 = tk.Menubutton(toolbar, text="Алгоритмы заполнения полигонов", relief=tk.RAISED)
    btn_segments5.menu = Menu(btn_segments5, tearoff=0)
    btn_segments5["menu"] = btn_segments5.menu

    # Опции в "Алгоритмы заполнения полигонов"
    btn_segments5.menu.add_command(label="Алгоритм растровой развёртки с упорядоченным списком рёбер", command=lambda: open_new_polygon_window(default_algorithm='graham', fill_algorithm='ordered_list_of_edges'))
    btn_segments5.menu.add_command(label="Алгоритм растровой развёртки с упорядоченным списком рёбер, использующий САР", command=lambda: open_new_polygon_window(default_algorithm='graham', fill_algorithm='ordered_list_with_active_list_of_edges'))
    btn_segments5.menu.add_command(label="Простой алгоритм заполнения с затравкой", command=lambda: open_new_polygon_window(default_algorithm='graham', fill_algorithm='simple'))
    btn_segments5.menu.add_command(label="Построчный алгоритм заполнения с затравкой", command=lambda: open_new_polygon_window(default_algorithm='graham', fill_algorithm='line_by_line'))
    btn_segments5.pack(side=tk.LEFT, padx=2, pady=2)
    ...
    label6 = Label(root, text="Триангуляция Делоне и диаграмма Вороного", font=("Arial",14))
    label6.pack(pady=10)

    button_frame = Frame(root)
    button_frame.pack()

    btn14 = Button(button_frame, text="Триангуляция Делоне", command=open_triangulation_window)
    btn14.pack(side="left", padx=5)

    btn15 = Button(button_frame, text="Диаграмма Вороного", command=open_input_window)
    btn15.pack(side="left", padx=5)
