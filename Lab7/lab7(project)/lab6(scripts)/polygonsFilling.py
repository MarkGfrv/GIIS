        ...
        for i in range(1, max_y + 1):
            ...
            for j in range(len(self.pol) - 1):
                if self.pol[j][1] == self.pol[j+1][1]:
                    continue
                intersection_points = find_intersect_with_concrete_line([self.pol[j][0], self.pol[j+1][0]],
                                                                        [self.pol[j][1], self.pol[j+1][1]], i)
...
def main():
    ...
    pf.draw_by_intervals(pf3.with_ordered_list_of_edges())
    pf.draw_by_intervals(pf3.ordered_list_using_active_edges())
